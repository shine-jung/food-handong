...
function List({ searchText, categoryText, sortBy, restaurants, recommendId }) {
  ...
  const filteredList = restaurantList
    .filter((restaurant) => {
      return categoryText === "" || restaurant.category === categoryText;
    })
    .filter((restaurant) => {
      if (searchText === "") {
        return !(
          restaurant.category !== categoryText && restaurant.id === recommendId
        );
      } else {
        ...
        for (let i = 0; restaurant.tags && i < restaurant.tags.length; i++)
          if (restaurant.tags[i].toLowerCase().includes(searchText))
            return true;
        return (
